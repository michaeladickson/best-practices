...
- **Worktree diverged from origin/main** → do NOT force-push. Fetch, rebase onto origin/main, resolve any conflicts (usually trivial — this session's edits vs. the weekly `[automated]` commits from the scheduled Friday runs), push. Match the pattern the session already used. And judge "diverged" by **content, not count**: after a squash-merged PR, `rev-list --count` reports commits that already landed (squash breaks ancestry) — `git diff origin/main` decides whether anything is actually unlanded (both crumbl-ops and command-center carry this scar).
  - ⚠️ **That content check is scoped to THIS session's own work, checked now.** It holds only while main has not since touched the same files. Do not reuse it to decide whether an *older* branch landed: once main moves on, **all three** local tests report a merged branch as unmerged — ancestry (`merge-base --is-ancestor`), patch-id (`git cherry`), and the content diff alike. Measured 2026-09-01 triaging 42 command-center branches: 10 of the 12 flagged "unlanded work" were merged, and the tell was a branch that *was* main's own squash commit. For anything older than the current session the PR record is the only authority — `gh pr list -R <owner>/<repo> --state all --limit 300 --json headRefName,number,state`, delete only on `MERGED`. Match it with **awk, not `grep -P`**: Git Bash here is not a UTF-8 locale, so `grep -P` dies and every branch silently reads "no PR", which looks identical to "safe to delete".
  - ⚠️ **And a missing remote ref is not what "never pushed" looks like.** `gh pr merge --delete-branch` removes `refs/remotes/origin/<branch>`, so *every* branch landed the normal way fails a ref-existence test. Require **both** — no remote ref AND no PR record — before calling a branch unpushed. A merged PR whose remote ref is gone is an ordinary leftover: delete the local branch, never re-push it. This inversion is more expensive than the squash trap above, because the remedy "never pushed" prescribes is *push it now*, and pushing a merged branch is the one action here that is not free.
  - ⚠️ **`jq`'s `.[0]` on an empty array yields the string `null`, not nothing.** A per-branch lookup written `-q '.[0] | "\(.number)\t..."'` returns a literal `null null null` for a branch with no PR — non-empty, so it flows past every "no PR" guard and into whatever the classifier does with an unrecognised state, which then answers confidently and wrongly. Use `.[0] // empty`. And prove both directions against a throwaway canary (`git branch zz-canary <branch-ahead-of-main>` gives you commits with no remote and no PR; `git branch -D zz-canary` after) rather than reading the code — the two failure modes point opposite ways and only one of them destroys work.
