...
| **Telemetry** (feed report, citation discovery, heartbeats, GH usage, model pricing) | inside weekly run | wrapper step | `data/feed_report.md` regenerated weekly | — | Best-effort by design; dies with the digest |
| **Citation discovery** (`digest/citation_discovery.py`) | inside weekly run | wrapper step | new `Feed candidate:` issues; untouched ones self-close at 30d | — | Acceptance stays ~0 for a quarter. Baseline 2026-09-13: **2 of 9** worth adding (huggingface.co, arcprize.org) — a fine discovery hit rate; the failure mode was zero triage, not zero signal |
| **Model pricing watch** (`digest/model_pricing.py`) | inside weekly run | wrapper step | `data/model_pricing.json` `fetched_at` ≤ 9d | — | Watchlist goes a year with no move that changes a decision, or OpenRouter list rates stop tracking first-party (spot-check two vendors before killing) |
