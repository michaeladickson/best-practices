...
-   **For agents handling sensitive information, implement a hybrid execution model where inference and planning occur in the cloud, but tool calls and access to sensitive context (code, secrets, internal services) are executed on-premises within isolated environments.** To meet compliance and security requirements for sensitive data, separate cloud-based agent inference and planning from the execution of tool calls and interaction with internal services. This ensures that sensitive context, such as source code or secrets, remains within controlled, on-premises infrastructure.
-   **Implement a hybrid compute architecture that dynamically splits agent tasks, keeping sensitive information and associated processing local to the device using a 'Privacy Gate,' while leveraging cloud models for non-sensitive parts to optimize privacy and cost.** Utilize a 'Privacy Gate' (running locally) to identify and keep sensitive information on the user's device. This allows for dynamic splitting of complex agent tasks, with non-sensitive parts processed by powerful cloud models and sensitive portions handled by smaller local models, balancing privacy, performance, and cost.
-   **Implement robust migration processes to ensure the fidelity of an agent's accumulated 'personality,' context, and routines when moving between platforms or environments.** When transitioning agents, it is crucial to preserve not only explicit memory tiers but also the subtle behavioral nuances and learned patterns that constitute an agent's 'personality' and 'routines'. This ensures consistent performance and avoids loss of efficiency or undesirable behavioral shifts post-migration, maintaining the agent's operational integrity.
...
-   **Implement validation and integrity checks for AI-generated compaction summaries to prevent self-generated prompt injections or persona shifts.** Agent systems using compaction to manage context window limits must implement mechanisms to detect and mitigate self-generated instructions or persona shifts injected by the model itself during summarization. This ensures the integrity of the agent's internal memory and prevents unintended deviations from its core directives by guarding against internal memory contamination.
...
-   **When designing tool interfaces or retrieval contracts, structure complex data outputs (e.g., SQL query results) as named objects rather than positional arrays to enhance model interpretability and reduce ambiguity in context assembly.** To prevent 'weaker models' from losing track of data, ensure that structured outputs from tools or retrieval processes are provided as arrays of named objects (e.g., JSON objects with keys) rather than simple positional arrays. This explicit naming convention significantly improves model comprehension and reliability.
-   **Implement a unified AI control plane to integrate diverse enterprise systems and tools for agents, mitigating 'AI leakage'.** Modern enterprises face 'AI leakage' where siloed agents operate on fragmented information from disparate systems (e.g., CRM, ERP, FSM). A control plane ensures agents can coalesce capabilities and data from multiple trusted sources, providing a comprehensive context and preventing information fragmentation across operational tasks.
-   **Clearly delineate between tasks delegated for agent analysis and decisions requiring human judgment, structuring context accordingly.** Agents excel at data gathering, pattern recognition, and synthesizing information ('analysis'), but critical decisions often require human oversight ('judgment'). Define agent contracts and context assembly to support the agent's analytical role while explicitly reserving and facilitating human judgment for high-stakes decisions, preventing the blurring of these roles.
...
-   **Architect a top-level coordinator agent to orchestrate parallel work streams, actively managing context flow between delegated sub-agents and consolidating their combined results.** For complex, long-horizon tasks, a dedicated coordinator agent can break down goals, assign tasks to multiple sub-agents running in parallel sessions, and ensure relevant context is passed and aggregated efficiently. This allows for scalable and coherent execution across distributed agentic workflows, centralizing oversight and result synthesis by the main agent.
...
-   **Corrections already have homes** — behavioral feedback → cross-session memory; architectural choices → the decision journal; a domain hypothesis that failed → demote it in `hypotheses.md`. A catch-all *corrections* log fragments these.
-   **A scoped error log is not a catch-all.** The distinction is category plus an exit path, not the filename. crumbl-ops `knowledge/ERRORS.md` works because it admits only deterministic infra/code failures (Postgres semantics, `gcloud` behavior, a pytest fixture scope) — never behavioral or architectural corrections, which route to `.claude/memory/feedback_*.md` — and its header carries a graduation rule: *"Conclusions graduate into the relevant domain or procedural file."* That rule fires in practice; the row-constructor conclusion graduated into the `sql-schema-recon` skill, the env-var-wipe class into `credential-rotation`. Without the category limit it becomes a sink; without the graduation rule it becomes a write-only archive. A log that has both stays small — 7 entries over five months.
...
-   **Externalize domain-specific guidance (e.g., design principles, brand guidelines) into separate, loadable prompt files accessible to all agents, providing consistent 'institutional memory' and reducing recurring failure modes across diverse agent environments.** To ensure consistent behavior and reduce failures across agents, especially when operating outside an internal codebase, extract human judgment and domain expertise (like design rules) into public, loadable prompt files. This makes institutional memory portable and consumable by any agent or tool.
-   **Implement continuous self-improvement loops for agents by monitoring user edits and corrections, automatically identifying recurring friction points, acquiring new domain-specific terminology, and suggesting new skills or rule refinements.** Develop automated systems that observe user interactions, such as edits to agent outputs, to continuously improve agent performance. These loops can autonomously learn new internal jargon, detect patterns of inefficiency, and propose new skills or refine existing rules without explicit, manual feedback.
-   **Establish automated processes for continuously distilling and updating human judgment and expertise into agent skills and knowledge bases.** To offload repeatable decision-making and reduce human bottlenecks, proactively capture human expertise (e.g., from public statements, writings, workflows) and integrate it into agent skills. Automate the updates to these distilled skills to ensure agents always operate with the most current human-derived judgment.
...
-   **Mandate agents generating code to explicitly include their underlying reasoning, design choices, and alternatives considered within the output or associated metadata for human review.** AI-generated code often lacks the human-accessible context of intent and trade-offs, making review difficult and prone to errors. By requiring agents to articulate their reasoning alongside the code, human engineers gain critical insight into the agent's decision-making process, improving review quality and preventing the adoption of poorly reasoned solutions. This effectively makes the agent's 'thinking process' a form of provenance.
-   **Implement a 'propose-then-validate' operational pattern where AI agents suggest real-world actions, and deterministic code independently validates these proposals before execution.** For agents interacting with production systems or making consequential decisions (e.g., financial transactions), a critical safety measure is to separate the agent's generative 'proposals' from their actual 'execution.' Deterministic, auditable code should act as a gate, ensuring agent suggestions adhere to predefined rules, safety protocols, and business logic before any action is taken.
-   **Implement robust input sanitization and tokenization awareness to prevent adversarial context manipulation techniques like ASCII Smuggling.** Attackers are using invisible Unicode tag characters to split keywords, bypassing filters and altering how LLM tokenizers parse text. This technique, sometimes called ASCII Smuggling, creates a mismatch between human-readable and software-processed text, demanding explicit protection for context integrity.
-   **Implement rigorous context sanitization and validation to prevent instruction injection vulnerabilities in agent inputs and tool descriptions.** Attackers can exploit agents by embedding malicious instructions within seemingly benign context, such as public issue descriptions or unsanitized user input. Treat all incoming context, including tool documentation, as potentially hostile and validate it to prevent agents from misinterpreting text as executable commands or privileged instructions.
-   **Implement governed access programs to restrict specialized or high-risk agent capabilities (e.g., advanced cybersecurity models, automated patching tools) to trusted defenders or specific, authorized use cases.** Extend access control beyond data to include specialized agent capabilities or models that pose higher risks, such as those performing vulnerability detection or automated patching. Restrict access to these powerful tools to a vetted group of 'trusted defenders' to ensure responsible and secure deployment.
...
-   **Implement visual regression testing with tools like Playwright to enable agents to self-assess the correctness and quality of generated visual outputs against reference images or predefined standards.** For agents generating visual content (e.g., UIs, 3D scenes), integrate a visual testing framework like Playwright into the development loop. Capture screenshots from predetermined viewpoints and compare them against expected references or real-world photos to identify subtle visual errors or unintended changes.
